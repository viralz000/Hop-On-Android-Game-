...
public class Level4 extends AppCompatActivity  implements View.OnTouchListener {
    ...
    public boolean onTouch(View v, MotionEvent me) {
        if (me.getAction() == MotionEvent.ACTION_DOWN) {
            switch(v.getId()) {
                case R.id.backk:
                    startActivity(new Intent(Level4.this, Stage1.class));
                    break;
                case R.id.retry:
                    startActivity(new Intent(Level4.this, Level4.class));
                    break;
                case R.id.actionup:
                    int arr_ind = 0;
                    boolean done = false;
                            ...
                            if (grids.get(arr_ind).getX() == motor.getX() && grids.get(arr_ind).getY() == motor.getY()) {
                                if (map_val[j - 1][i] >= 5 && map_val[j - 1][i] <= 19) {
                                    if (map_val[j - 1][i] == 13 && map_val[j - 2][i] == 29) {
                                        ...
                                        replace++;
                                        if (replace == 1) {
                                            grids.get(132).setImageResource(images[map_val[j - 1][i]]);
                                        }
                                    }


                                    if (map_val[j - 1][i] == 13 && map_val[j - 2][i] == 31 && house) {
                                        customer2 = true;
                                        replace2++;
                                        if (replace2 == 1) {
                                            grids.get(140).setImageResource(images[map_val[j - 1][i]]);
                                        }
                                    }
                                    else if (map_val[j - 1][i] == 13 && map_val[j - 1][i] == 31 && customer) {
                                        Toast.makeText(getApplicationContext(),"dddddd", Toast.LENGTH_SHORT).show();
                                        startActivity(new Intent(Level4.this, Level4failed.class));
                                    }
                                    ...
                                    if (map_val[j-1][i] == 15 && map_val[j - 2][i] == 21 && customer2 && house) {
                                        house2 = true;
                                    }
                                    ...
                                    if (customer && customer2 && house) {
                                        motor.setImageDrawable(motortwoup);
                                        if (house2) {
                                            startActivity(new Intent(Level4.this, Level4success.class));
                                        }
                                    }
                                    break;
                                }
                        ...
                        for (int i = 0; i < CURR_COL; i++) {
                            if (grids.get(arr_ind1).getX() == motor.getX() && grids.get(arr_ind1).getY() == motor.getY()) {
                                if (map_val[j][i - 1] >= 5 && map_val[j][i - 1] <= 19) {
                                    ...
                                    if (map_val[j][i-1] == 13 && map_val[j - 1][i-1] == 31 && house) {
                                        customer2 = true;
                                        replace2++;
                                        if (replace2 == 1) {
                                            grids.get(140).setImageResource(images[map_val[j - 2][i - 1]]);
                                        }
                                    }
                                    if (map_val[j][i - 1] == 15 && map_val[j - 1][i - 1] == 22 && customer) {
                                        house = true;
                                    }
                                    if (map_val[j][i - 1] == 15 && map_val[j - 1][i - 1] == 21 && customer2 && house) {
                                        house2 = true;
                                    }
                                    for (int h = 1; h<stack.size(); h++) {

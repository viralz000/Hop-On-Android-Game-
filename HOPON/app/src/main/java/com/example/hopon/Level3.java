...
public class Level3 extends AppCompatActivity  implements View.OnTouchListener {
    ...
    final int map_val[][] = {{28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28},
            ...
            {28, 14, 0, 14, 0, 0, 14, 31, 27, 14, 0, 14, 28},
    ...
    public boolean onTouch(View v, MotionEvent me) {
        if (me.getAction() == MotionEvent.ACTION_DOWN) {
            switch(v.getId()) {
                case R.id.backk:
                    startActivity(new Intent(Level3.this, Stage1.class));
                    break;
                case R.id.retry:
                    startActivity(new Intent(Level3.this, Level3.class));
                    break;
                case R.id.actionup:
                    int arr_ind = 0;
                    boolean done = false;
                        ...
                        for (int i = 0; i < CURR_COL; i++) {
                            if (grids.get(arr_ind).getX() == motor.getX() && grids.get(arr_ind).getY() == motor.getY()) {
                                if (map_val[j - 1][i] >= 5 && map_val[j - 1][i] <= 19) {
                                    ...
                                    if (map_val[j-1][i] == 15 && map_val[j - 2][i] == 22 && customer && customer2 && customer3) {
                                        house = true;
                                    }
                                    for (int h = 1; h<stack.size(); h++) {
                                        if (grids.get(arr_ind) == stack.get(h)) {
                                            startActivity(new Intent(Level3.this, Level3failed.class));
                                            break;
                                        }
                                    }
                                    ...
                                    if (customer && customer2 && customer3) {
                                        motor.setImageDrawable(motortwoup);
                                        if (house) {
                                            startActivity(new Intent(Level3.this, Level3success.class));
                                        }
                                    }
                                    break;
                                }
                            ...
                            if (grids.get(arr_ind1).getX() == motor.getX() && grids.get(arr_ind1).getY() == motor.getY()) {
                                if (map_val[j][i - 1] >= 5 && map_val[j][i - 1] <= 19) {
                                    ...
                                    if (map_val[j][i-1] == 13 && map_val[j - 1][i-1] == 31) {
                                        ...
                                        replace3++;
                                        if (replace3 == 1) {
                                            grids.get(111).setImageResource(images[map_val[j - 3][i]]);
                                        }
                                    }
                                    if (map_val[j][i - 1] == 15 && map_val[j - 1][i - 1] == 22 && customer && customer2 && customer3) {
                                        house = true;
                                    }
                                    for (int h = 1; h<stack.size(); h++) {
                            ...
                            if (grids.get(arr_ind2).getX() == motor.getX() && grids.get(arr_ind2).getY() == motor.getY()) {
                                if (map_val[j][i + 1] >= 5 && map_val[j][i + 1] <= 19) {
                                    ...
                                    if (map_val[j][i+1] == 13 && map_val[j - 1][i+1] == 31) {
                                        ...
                                        replace3++;
                                        if (replace3 == 1) {
                                            grids.get(111).setImageResource(images[map_val[j - 3][i + 1]]);
                                        }
                                    }
                                    if (map_val[j][i + 1] == 15 && map_val[j - 1][i + 1] == 22 && customer && customer2 && customer3) {
                                        house = true;

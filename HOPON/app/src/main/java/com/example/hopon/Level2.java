...
public class Level2 extends AppCompatActivity  implements View.OnTouchListener {
    ...
    private ImageView motor, redmotor;

    //angles for blue motor
    ...
    //angles for red motor
    private Drawable redmotorup, redmotorright, redmotorleft, redmotorodwn, redmotortwoup, redmotortwoleft,
    redmotortworight, redmotortwodown;
    private float redmotorX, redmotorY;

    private boolean action_up, action_down, action_right, action_left, nextmotor;
    private float x;
    private float y;
    ...
    final int map_val[][] = {{28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28},
            ...
            {28, 14, 30, 14, 0, 0, 14, 0, 29, 14, 0, 14, 28},
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //angles for red motor
        redmotor = findViewById(R.id.yellowmotor);
        redmotorup = getResources().getDrawable(R.drawable.redtopsingleup);
    ...
    public boolean onTouch(View v, MotionEvent me) {
        if (me.getAction() == MotionEvent.ACTION_DOWN) {
            switch(v.getId()) {
                case R.id.backk:
                    startActivity(new Intent(Level2.this, Stage1.class));
                    break;
                case R.id.retry:
                        ...
                        for (int i = 0; i < CURR_COL; i++) {
                            if (grids.get(arr_ind).getX() == motor.getX() && grids.get(arr_ind).getY() == motor.getY()) {
                                if (map_val[j - 1][i] >= 5 && map_val[j - 1][i] <= 19) {
                                    ...
                                    if (map_val[j - 2][i] == 22 && customer && customer2) {
                                        house = true;
                                    }
                                    for (int h = 1; h<stack.size(); h++) {
                                        if (grids.get(arr_ind) == stack.get(h)) {
                                            startActivity(new Intent(Level2.this, Level2failed.class));
                                            break;
                                        }
                                    }
                                    ...
                                    if (customer && customer2) {
                                        ...
                                        if (house) {
                                            startActivity(new Intent(Level2.this, Level2success.class));
                                        }
                                    }
                                    break;
                                }
                            ...
                            if (grids.get(arr_ind1).getX() == motor.getX() && grids.get(arr_ind1).getY() == motor.getY()) {
                                if (map_val[j][i - 1] >= 5 && map_val[j][i - 1] <= 19) {
                                    ...
                                    if (map_val[j][i-1] == 13 && map_val[j - 1][i-1] == 30) {
                                        ...
                                        replace2++;
                                        if (replace2 == 1) {
                                            grids.get(132).setImageResource(images[map_val[j - 1][i + 1]]);
                                        }
                                    }
                                    if (map_val[j][i - 1] == 15 && map_val[j - 1][i - 1] == 22 && customer && customer2) {
                                        house = true;
                                    }
                                    for (int h = 1; h<stack.size(); h++) {
                            ...
                            if (grids.get(arr_ind2).getX() == motor.getX() && grids.get(arr_ind2).getY() == motor.getY()) {
                                if (map_val[j][i + 1] >= 5 && map_val[j][i + 1] <= 19) {
                                    ...
                                    if (map_val[j][i + 1] == 13 && map_val[j - 1][i + 1] == 30) {
                                        ...
                                        replace2++;
                                        if (replace2 == 1) {
                                            grids.get(132).setImageResource(images[map_val[j - 2][i+1]]);
                                        }
                                    }
                                    if (map_val[j][i + 1] == 15 && map_val[j - 1][i + 1] == 22 && customer && customer2) {
                                        house = true;
                        ...
                        for (int i = 0; i < CURR_COL; i++) {
                            if (grids.get(arr_ind3).getX() == motor.getX() && grids.get(arr_ind3).getY() == motor.getY()) {
                                if (map_val[j + 1][i] >= 5 && map_val[j + 1][i] <= 19) {
                                    ...
                                    if (map_val[j + 1][i] == 15 && map_val[j + 2][i] == 22 && customer & customer2) {
                                        house = true;
                                    }
                                    for (int h = 1; h<stack.size(); h++) {

...
public class Level9 extends AppCompatActivity  implements View.OnTouchListener {
    ...
    final int map_val[][] = {{28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28},
            ...
            {28, 28, 14, 0, 0, 14, 31, 14, 27, 0, 14, 28, 28},
    ...
    public boolean onTouch(View v, MotionEvent me) {
        ...
        if (me.getAction() == MotionEvent.ACTION_DOWN) {
            switch(v.getId()) {
                case R.id.backk:
                    startActivity(new Intent(Level9.this, Stage1.class));
                    break;
                case R.id.retry:
                    startActivity(new Intent(Level9.this, Level9.class));
                    break;
                case R.id.actionup:
                    int arr_ind = 0;
                    boolean done = false;
                            ...
                            if (grids.get(arr_ind).getX() == motor.getX() && grids.get(arr_ind).getY() == motor.getY()) {
                                if (map_val[j - 1][i] >= 5 && map_val[j - 1][i] <= 19) {
                                    if (map_val[j-1][i] == 13 && map_val[j-2][i] == 31) {
                                        ...
                                        replace++;
                                        if (replace == 1) {
                                            grids.get(135).setImageResource(images[map_val[j - 1][i]]);
                                        }
                                    }
                                    if (map_val[j-2][i] == 24 && customer) {
                                        house = true;
                                    }
                                    for (int h = 1; h<stack.size(); h++) {
                                        if (grids.get(arr_ind) == stack.get(h)) {
                                            startActivity(new Intent(Level9.this, Level9failed.class));
                                            break;
                                        }
                                    }
                                    ...
                                    if (customer) {
                                        ...
                                        if (house) {
                                            startActivity(new Intent(Level9.this, Level9success.class));
                                        }
                                    }
                                    break;
                                }
                        ...
                        for (int i = 0; i < CURR_COL; i++) {
                            if (grids.get(arr_ind1).getX() == motor.getX() && grids.get(arr_ind1).getY() == motor.getY()) {
                                if (map_val[j][i-1] >= 5 && map_val[j][i-1] <= 19) {
                                    ...
                                    if (map_val[j][i-1] == 13 && map_val[j-1][i-1] == 24 && customer) {
                                        house = true;
                                    }
                                    for (int h = 1; h<stack.size(); h++) {
                        ...
                        for (int i = 0; i < CURR_COL; i++) {
                            if (grids.get(arr_ind3).getX() == motor.getX() && grids.get(arr_ind3).getY() == motor.getY()) {
                                if (map_val[j+1][i] >= 5 && map_val[j+1][i] <= 19 ) {
                                    ...
                                    if (map_val[j+1][i] == 15 && map_val[j+2][i] == 24 && customer) {
                                        house = true;
                                    }

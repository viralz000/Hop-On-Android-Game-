...
public class Level5 extends AppCompatActivity  implements View.OnTouchListener {
    ...
    final int map_val[][] = {{28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28},
            ...
            {28, 0, 0, 14, 30, 18, 13, 16, 0, 14, 0, 0, 28},
    ...
    public boolean onTouch(View v, MotionEvent me) {
        ...
        if (me.getAction() == MotionEvent.ACTION_DOWN) {
            switch(v.getId()) {
                case R.id.backk:
                    startActivity(new Intent(Level5.this, Stage1.class));
                    break;
                case R.id.retry:
                    startActivity(new Intent(Level5.this, Level5.class));
                    break;
                case R.id.actionup:
                    int arr_ind = 0;
                    boolean done = false;
                        ...
                        for (int i = 0; i < CURR_COL; i++) {
                            if (grids.get(arr_ind).getX() == motor.getX() && grids.get(arr_ind).getY() == motor.getY()) {
                                if (map_val[j - 1][i] >= 5 && map_val[j - 1][i] <= 19) {
                                    if (map_val[j-1][i] == 13 && map_val[j-2][i] == 30) {
                                        customer = true;
                                        replace++;
                                        if (replace == 1) {
                                    ...
                                    for (int h = 1; h<stack.size(); h++) {
                                        if (grids.get(arr_ind) == stack.get(h)) {
                                            startActivity(new Intent(Level5.this, Level5failed.class));
                                            break;
                                        }
                                    }
                                    ...
                                    if (customer) {
                                        ...
                                        if (house) {
                                            startActivity(new Intent(Level5.this, Level5success.class));
                                        }
                                    }
                                    break;
                                }
                            ...
                            if (grids.get(arr_ind1).getX() == motor.getX() && grids.get(arr_ind1).getY() == motor.getY()) {
                                if (map_val[j][i-1] >= 5 && map_val[j][i-1] <= 19) {
                                    if (map_val[j][i-1] == 13 && map_val[j-1][i-1] == 30) {
                                        ...
                                        replace++;
                                        if (replace == 1) {
                                            grids.get(134).setImageResource(images[map_val[j-1][i+3]]);
                                        }
                                    }
                            ...
                            if (grids.get(arr_ind3).getX() == motor.getX() && grids.get(arr_ind3).getY() == motor.getY()) {
                                if (map_val[j+1][i] >= 5 && map_val[j+1][i] <= 19 ) {
                                    if (map_val[j+1][i] == 13 && map_val[j+2][i] == 29) {
                                        ...
                                        replace++;
                                        if (replace == 1) {
                                            grids.get(134).setImageResource(images[map_val[j - 1][i]]);
                                        }
                                    }
                                    if (map_val[j+2][i] == 23 && customer) {
                                        house = true;

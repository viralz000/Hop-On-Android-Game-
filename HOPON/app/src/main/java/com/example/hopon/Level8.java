...
public class Level8 extends AppCompatActivity  implements View.OnTouchListener {
    ...
    final int map_val[][] = {{28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28},
            ...
            {28, 0, 0, 14, 0, 18, 13, 16, 31, 14, 0, 0, 28},
            {28, 0, 0, 18, 13, 16, 0, 18, 13, 16, 0, 0, 28},
            {28, 0, 0, 14, 30, 14, 0, 14, 0, 14, 0, 0, 28},
    ...
    public boolean onTouch(View v, MotionEvent me) {
        ...
        if (me.getAction() == MotionEvent.ACTION_DOWN) {
            switch(v.getId()) {
                case R.id.backk:
                    startActivity(new Intent(Level8.this, Stage1.class));
                    break;
                case R.id.retry:
                    startActivity(new Intent(Level8.this, Level8.class));
                    break;
                case R.id.actionup:
                    int arr_ind = 0;
                    boolean done = false;
                        ...
                        for (int i = 0; i < CURR_COL; i++) {
                            if (grids.get(arr_ind).getX() == motor.getX() && grids.get(arr_ind).getY() == motor.getY()) {
                                if (map_val[j - 1][i] >= 5 && map_val[j - 1][i] <= 19) {
                                    if (map_val[j-2][i] == 23 && customer) {
                                        house = true;
                                    }
                                    if (map_val[j-2][i] == 20 && customer2 && customer && house) {
                                        house2 = true;
                                    }
                                    for (int h = 1; h<stack.size(); h++) {
                                        if (grids.get(arr_ind) == stack.get(h)) {
                                            startActivity(new Intent(Level8.this, Level8failed.class));
                                            break;
                                        }
                                    }
                                    ...
                                    if (customer || customer2) {
                                        ...
                                        if (house && house2) {
                                            startActivity(new Intent(Level8.this, Level8success.class));
                                        }
                                    }
                                    break;
                                }
                            ...
                            if (grids.get(arr_ind1).getX() == motor.getX() && grids.get(arr_ind1).getY() == motor.getY()) {
                                if (map_val[j][i-1] >= 5 && map_val[j][i-1] <= 19) {
                                    if (map_val[j][i-1] == 13 && map_val[j-1][i-1] == 30) {
                                        ...
                                        replace++;
                                        if (replace == 1) {
                                            grids.get(160).setImageResource(images[map_val[j-1][i+1]]);
                                        }
                                    }
                                    ...
                                    if (map_val[j-1][i-1] == 23 && customer) {
                                        house = true;
                                    }
                                    for (int h = 1; h<stack.size(); h++) {
                                        b = h;
                            ...
                            if (grids.get(arr_ind2).getX() == motor.getX() && grids.get(arr_ind2).getY() == motor.getY()) {
                                if (map_val[j][i+1] >= 5 && map_val[j][i+1] <= 19) {
                                    if (map_val[j][i+1] == 13 && map_val[j-1][i+1] == 30) {
                                        ...
                                        replace++;
                                        if (replace == 1) {
                                            grids.get(160).setImageResource(images[map_val[j - 1][i+3]]);
                                        }
                                    }
                                    if (map_val[j][i+1] == 13 && map_val[j-1][i+1] == 31 && house) {
                                        customer2 = true;
